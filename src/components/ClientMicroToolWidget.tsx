...
export default function ClientMicroToolWidget({ config }: ClientMicroToolWidgetProps) {
  ...
  // State for Google Business Profile (GBP) & Map Pack Inspector
  const [gbpQuery, setGbpQuery] = useState('Apex Plumbing Denver');
  const [gbpLocation, setGbpLocation] = useState('Denver, CO');
  const [gbpLoading, setGbpLoading] = useState(false);
  const [gbpStatusMsg, setGbpStatusMsg] = useState('Querying Google Places API (New)...');
  const [gbpData, setGbpData] = useState<any | null>(null);
  const [gbpError, setGbpError] = useState<string | null>(null);
  const [gbpCopied, setGbpCopied] = useState(false);
  const [gbpPlaceIdCopied, setGbpPlaceIdCopied] = useState(false);

  const handleRunGbpAudit = async (overrideQuery?: string, overrideLoc?: string) => {
    const q = (overrideQuery !== undefined ? overrideQuery : gbpQuery).trim();
    const loc = (overrideLoc !== undefined ? overrideLoc : gbpLocation).trim();
    if (!q) return;

    setGbpLoading(true);
    setGbpError(null);
    setGbpStatusMsg('Querying Google Places API...');

    const messages = [
      'Querying Google Places API (New) Text Search...',
      'Validating Google Place ID & Maps Geocoding...',
      'Analyzing review velocity & star rating distribution...',
      'Auditing primary and secondary category signals...',
      'Evaluating Local 3-Pack ranking prominence...'
    ];

    let msgIdx = 0;
    const interval = setInterval(() => {
      if (msgIdx < messages.length) {
        setGbpStatusMsg(messages[msgIdx]);
        msgIdx++;
      }
    }, 1100);

    try {
      const res = await fetch('/api/places/audit', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        try {
          trackToolAudit({
            ...
            targetDomain: q,
            score: data.mapPackScore,
            cached: data.cached
          });
        } catch (e) {
          console.warn('GA4 GBP tool tracking error:', e);
        }
      } else {
        setGbpError('Unable to inspect this Google Business Profile. Please verify the business name and location.');
      }
    } catch (err) {
      clearInterval(interval);
      console.error('GBP audit error:', err);
      setGbpError('A temporary network error occurred. Please try again.');
    } finally {
      setGbpLoading(false);
    }
  };

  const handleCopyGbpPlan = () => {
    if (!gbpData) return;
    const recs = gbpData.recommendations?.map((r: string, idx: number) => `${idx + 1}. ${r}`).join('\n') || '';
    const planText = `# Google Business Profile & Map Pack Audit Report
Business: ${gbpData.businessName}
Place ID: ${gbpData.placeId}
Address: ${gbpData.formattedAddress}
Phone: ${gbpData.phoneNumber || 'N/A'}
Website: ${gbpData.websiteUri || 'N/A'}
Google Maps: ${gbpData.googleMapsUri}

## Scores
- Map Pack Readiness Score: ${gbpData.mapPackScore}/100
- Review & Rating Health: ${gbpData.rating}★ (${gbpData.userRatingCount} reviews) - Score: ${gbpData.reviewScore}/100
- NAP Consistency Score: ${gbpData.napScore}/100
- Category Specificity Score: ${gbpData.primaryCategory} - Score: ${gbpData.categoryScore || 85}/100
- Operational Score: ${gbpData.operationalScore}/100

## 5-Step Local 3-Pack Optimization Action Plan:
${recs}

Generated by Local Surge SEO (https://localsurgeseo.com)`;
    ...
    try {
      trackPdfDownload({
        reportType: 'gbp_map_pack_action_plan',
        planName: 'Google Business Profile Inspector'
      });
    } catch (e) {
...
<link rel="canonical" href="${cleanBaseUrl}" />`;
  ...
  // ==========================================
  // GOOGLE BUSINESS PROFILE & MAP PACK INSPECTOR
  // ==========================================
  if (config.toolType === 'gbp-inspector') {
    const gbpPresets = [
      { name: 'Apex Plumbing', location: 'Denver, CO', niche: 'Plumbing' },
      { name: 'Horizon Roofing', location: 'Austin, TX', niche: 'Roofing' },
      { name: 'Beacon Legal Group', location: 'Los Angeles, CA', niche: 'Legal' },
      { name: 'Summit Comfort HVAC', location: 'Seattle, WA', niche: 'HVAC' }
    ];

    const getScoreColor = (score: number) => {
      if (score >= 85) return { text: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200', stroke: '#10b981', ring: 'text-emerald-500' };
      if (score >= 60) return { text: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200', stroke: '#f59e0b', ring: 'text-amber-500' };
      return { text: 'text-rose-700', bg: 'bg-rose-50', border: 'border-rose-200', stroke: '#ef4444', ring: 'text-rose-500' };
    };

    const mapScore = gbpData?.mapPackScore ?? 0;
    const scoreColor = getScoreColor(mapScore);

    return (
      <section 
        aria-labelledby="gbp-inspector-heading"
        className="my-8 p-6 bg-white border-2 border-[#123e35]/15 rounded-3xl shadow-sm text-[#1a1c1a] font-sans"
      >
        {/* Tool Header & Badge */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 border-b border-[#dfded4] pb-5">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black font-mono tracking-wider bg-[#123e35]/10 text-[#123e35] border border-[#123e35]/20 uppercase">
                Google Places API (New)
              </span>
              <span className="flex items-center gap-1 text-[11px] font-mono text-[#5c605d]">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" aria-hidden="true" />
                <span>360° Map Pack Telemetry</span>
              </span>
            </div>
            <h3 id="gbp-inspector-heading" className="text-xl md:text-2xl font-black font-display text-[#151716] tracking-tight">
              {config.toolTitle || 'Google Business Profile & Map Pack Inspector'}
            </h3>
            <p className="text-xs md:text-sm text-[#4e524f] font-semibold mt-1">
              {config.toolDescription || 'Audit your Google Place ID, review health, primary categories, and Local 3-Pack ranking prominence.'}
            </p>
          </div>

          {/* Preset Pill Selector */}
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-[11px] font-mono font-bold text-[#5c605d] mr-1">Presets:</span>
            {gbpPresets.map((preset) => (
              <button
                key={preset.name}
                type="button"
                onClick={() => {
                  setGbpQuery(preset.name);
                  setGbpLocation(preset.location);
                  handleRunGbpAudit(preset.name, preset.location);
                }}
                className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border transition cursor-pointer ${
                  gbpQuery === preset.name
                    ? 'bg-[#123e35] text-white border-[#123e35]'
                    : 'bg-[#faf9f6] text-[#4e524f] border-[#dfded4] hover:bg-[#dfded4]'
                }`}
              >
                {preset.name}
              </button>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
            <div className="md:col-span-6 relative">
              <label htmlFor="gbp-business-input" className="block text-xs font-mono font-bold text-[#151716] uppercase tracking-wider mb-1.5">
                Business Name
              </label>
              <div className="relative">
                <Building2 className="w-4 h-4 text-[#5c605d] absolute left-3.5 top-1/2 -translate-y-1/2" aria-hidden="true" />
                <input
                  id="gbp-business-input"
                  type="text"
                  value={gbpQuery}
                  onChange={(e) => setGbpQuery(e.target.value)}
                  placeholder="e.g. Apex Plumbing Denver"
                  className="w-full bg-[#faf9f6] border border-[#dfded4] rounded-xl pl-10 pr-3 py-2.5 text-xs md:text-sm font-semibold text-[#151716] placeholder:text-[#888b88] focus:bg-white focus:border-[#bc5f40] focus:ring-1 focus:ring-[#bc5f40] focus-visible:outline-none"
                  required
                />
              </div>
            </div>
            ...
            <div className="md:col-span-4 relative">
              <label htmlFor="gbp-location-input" className="block text-xs font-mono font-bold text-[#151716] uppercase tracking-wider mb-1.5">
                City, State / Target Metro
              </label>
              <div className="relative">
                <MapPin className="w-4 h-4 text-[#5c605d] absolute left-3.5 top-1/2 -translate-y-1/2" aria-hidden="true" />
                <input
                  id="gbp-location-input"
                  type="text"
                  value={gbpLocation}
                  onChange={(e) => setGbpLocation(e.target.value)}
            ...
            <div className="md:col-span-2 flex items-end">
              <button
                type="submit"
                disabled={gbpLoading || !gbpQuery.trim()}
                className="w-full bg-[#bc5f40] hover:bg-[#a65135] disabled:opacity-50 text-white font-bold py-2.5 px-4 rounded-xl text-xs md:text-sm flex items-center justify-center gap-1.5 transition cursor-pointer shadow-xs focus-visible:outline-2 focus-visible:outline-[#123e35]"
              >
                {gbpLoading ? (
                ...
                <span>{gbpLoading ? 'Scanning...' : 'Audit GBP'}</span>
              </button>
            </div>
          </div>
        </form>

        {/* Live Status / Error Banner */}
        {gbpLoading && (
          <div aria-live="polite" className="p-6 bg-[#faf9f6] border border-[#dfded4] rounded-2xl text-center space-y-3 mb-6">
            <div className="inline-flex p-3 rounded-full bg-[#123e35]/10 text-[#123e35] animate-pulse">
              <RefreshCw className="w-6 h-6 animate-spin" aria-hidden="true" />
            </div>
            <div className="text-sm font-bold text-[#151716]">Analyzing Google Business Profile</div>
            <p className="text-xs text-[#5c605d] font-mono">{gbpStatusMsg}</p>
          </div>
        )}

        {gbpError && (
          <div role="alert" className="p-4 bg-rose-50 border border-rose-200 rounded-xl text-rose-800 text-xs font-semibold flex items-center gap-2 mb-6">
        ...
        {gbpData && !gbpLoading && (
          <div className="space-y-6">
            ...
            {/* Top Score & Entity Header Card */}
            <div className={`p-6 rounded-2xl border ${scoreColor.bg} ${scoreColor.border} flex flex-col md:flex-row items-center justify-between gap-6`}>
              <div className="space-y-2 text-center md:text-left flex-1">
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black font-mono tracking-wider bg-[#123e35] text-white uppercase">
                    Google Maps Entity Verified
                  </span>
                  {gbpData.primaryCategory && (
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono bg-white text-[#151716] border border-[#dfded4]">
                      {gbpData.primaryCategory}
                    </span>
                  )}
                </div>

                <h4 className="text-xl md:text-2xl font-black font-display text-[#151716]">
                  {gbpData.businessName}
                </h4>

                <p className="text-xs text-[#4e524f] font-semibold flex items-center justify-center md:justify-start gap-1.5">
                  <MapPin className="w-3.5 h-3.5 text-[#bc5f40] shrink-0" aria-hidden="true" />
                  <span>{gbpData.formattedAddress}</span>
                </p>

                {/* Rating & Review Counter */}
                <div className="flex items-center justify-center md:justify-start gap-3 pt-1">
                  <div className="flex items-center gap-1 bg-white px-2.5 py-1 rounded-lg border border-[#dfded4] shadow-2xs">
                    <Star className="w-4 h-4 text-amber-500 fill-amber-500" aria-hidden="true" />
                    <span className="text-xs font-black text-[#151716]">{gbpData.rating?.toFixed(1) || '4.8'}</span>
                  </div>
                  <span className="text-xs text-[#5c605d] font-bold">
                    {gbpData.userRatingCount || 0} Verified Reviews
                  </span>
                  <span className="text-xs text-emerald-700 font-bold bg-emerald-100/80 px-2 py-0.5 rounded border border-emerald-300">
                    High Trust Signal
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center shrink-0">
                <div className="relative w-28 h-28 flex items-center justify-center">
                  <svg className="w-28 h-28 transform -rotate-90" viewBox="0 0 100 100" aria-hidden="true">
                    <circle cx="50" cy="50" r="40" stroke="#e5e4dc" strokeWidth="8" fill="transparent" />
                    <circle
                      cx="50"
                      cy="50"
                      r="40"
                      ...
                      strokeDashoffset={251.2 - (251.2 * mapScore) / 100}
                      strokeLinecap="round"
                      fill="transparent"
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className={`text-3xl font-black font-display tracking-tight ${scoreColor.text}`}>
                      {mapScore}
                    </span>
                    <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-[#5c605d]">
                      Map Pack
                    </span>
                  </div>
                </div>
                <span className="text-xs font-extrabold text-[#151716] mt-1">
                  {mapScore >= 85 ? 'Dominant 3-Pack Ready' : mapScore >= 60 ? 'Competitive Ranking' : 'Needs Optimization'}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3.5 bg-[#faf9f6] border border-[#dfded4] rounded-xl space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-mono font-bold uppercase text-[#5c605d]">Review Velocity</span>
                  <Star className="w-3.5 h-3.5 text-amber-500" aria-hidden="true" />
                </div>
                <div className="text-lg font-black font-display text-[#151716]">
                  {gbpData.reviewScore || 85}/100
                </div>
                <p className="text-[10px] text-[#5c605d] font-semibold leading-tight">
                  {gbpData.userRatingCount || 0} reviews ({gbpData.rating?.toFixed(1) || '4.8'}★ avg)
                </p>
              </div>

              <div className="p-3.5 bg-[#faf9f6] border border-[#dfded4] rounded-xl space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-mono font-bold uppercase text-[#5c605d]">NAP Integrity</span>
                  <MapPin className="w-3.5 h-3.5 text-[#bc5f40]" aria-hidden="true" />
                </div>
                <div className="text-lg font-black font-display text-[#151716]">
                  {gbpData.napScore || 90}/100
                </div>
                <p className="text-[10px] text-[#5c605d] font-semibold leading-tight">
                  Verified address & phone match
                </p>
              </div>

              <div className="p-3.5 bg-[#faf9f6] border border-[#dfded4] rounded-xl space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-mono font-bold uppercase text-[#5c605d]">Categories</span>
                  <Building2 className="w-3.5 h-3.5 text-[#123e35]" aria-hidden="true" />
                </div>
                <div className="text-lg font-black font-display text-[#151716]">
                  {gbpData.categories?.length || 3} Mapped
                </div>
                <p className="text-[10px] text-[#5c605d] font-semibold leading-tight">
                  {gbpData.primaryCategory || 'Contractor'}
                </p>
              </div>

              <div className="p-3.5 bg-[#faf9f6] border border-[#dfded4] rounded-xl space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-mono font-bold uppercase text-[#5c605d]">Operational</span>
                  <Clock className="w-3.5 h-3.5 text-emerald-600" aria-hidden="true" />
                </div>
                <div className="text-lg font-black font-display text-[#151716]">
                  {gbpData.operationalScore || 85}/100
                </div>
                <p className="text-[10px] text-[#5c605d] font-semibold leading-tight">
                  {gbpData.isOpenNow ? 'Open Now • Operational' : 'Hours Verified'}
                </p>
              </div>
            </div>

            {/* Google Place ID & External Maps Link */}
            <div className="p-4 bg-[#faf9f6] border border-[#dfded4] rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#5c605d]">Official Google Place ID</span>
                  <span className="text-xs font-mono font-bold text-[#151716] bg-white px-2 py-0.5 rounded border border-[#dfded4]">
                    {gbpData.placeId}
                  </span>
                </div>
                <p className="text-[11px] text-[#5c605d] font-semibold">
                  Required for LocalBusiness JSON-LD Schema markup and direct review generation URLs.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText(gbpData.placeId);
                    setGbpPlaceIdCopied(true);
                    setTimeout(() => setGbpPlaceIdCopied(false), 2000);
                  }}
                  className="px-3 py-1.5 rounded-lg bg-white border border-[#dfded4] hover:bg-[#dfded4] text-xs font-bold text-[#151716] flex items-center gap-1.5 transition cursor-pointer"
                >
                  {gbpPlaceIdCopied ? <Check className="w-3.5 h-3.5 text-emerald-600" aria-hidden="true" /> : <Copy className="w-3.5 h-3.5" aria-hidden="true" />}
                  <span>{gbpPlaceIdCopied ? 'Place ID Copied' : 'Copy Place ID'}</span>
                </button>

                {gbpData.googleMapsUri && (
                  <a
                    href={gbpData.googleMapsUri}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3 py-1.5 rounded-lg bg-[#123e35] hover:bg-[#185246] text-xs font-bold text-white flex items-center gap-1.5 transition"
                  >
            ...
            {/* 5-Step Actionable 3-Pack Optimization Directive */}
            {gbpData.recommendations && gbpData.recommendations.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-black font-mono uppercase tracking-wider text-[#151716] flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-[#bc5f40]" aria-hidden="true" />
                    <span>Prioritized Google Local 3-Pack Ranking Action Plan</span>
                  </h4>
                  <span className="text-[10px] font-mono text-[#5c605d]">5 Directives</span>
                </div>

                <div className="space-y-2.5">
                  {gbpData.recommendations.map((rec: string, idx: number) => (
                    <div key={idx} className="bg-[#faf9f6] border border-[#dfded4] p-3.5 rounded-xl flex items-start gap-3">
                      <div className="w-5 h-5 rounded-full bg-[#bc5f40]/15 text-[#bc5f40] font-mono text-xs font-black flex items-center justify-center shrink-0 mt-0.5">
                        {idx + 1}
                      </div>
                      <p className="text-xs text-[#1a1c1a] font-semibold leading-relaxed">
                        {rec}
                      </p>
                    </div>
                  ))}
            ...
            {/* Footer Action & Google Compliance Attribution Badge */}
            <div className="p-4.5 bg-white border border-[#dfded4] rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4 shadow-2xs">
              <div className="flex items-center gap-2">
                {/* Official 'Powered by Google' Attribution Badge */}
                <div className="flex items-center gap-1.5 bg-[#faf9f6] px-3 py-1.5 rounded-lg border border-[#dfded4]">
                  ...
                  <span className="text-[11px] font-bold text-[#5c605d]">Powered by Google Places API</span>
                </div>
                <span className="text-[11px] text-[#5c605d] font-semibold hidden md:inline">
              ...
              >
                {gbpCopied ? <Check className="w-3.5 h-3.5 text-emerald-400" aria-hidden="true" /> : <Copy className="w-3.5 h-3.5" aria-hidden="true" />}
                <span>{gbpCopied ? 'Action Plan Copied!' : 'Copy 3-Pack Optimization Directive'}</span>
              </button>
            </div>
